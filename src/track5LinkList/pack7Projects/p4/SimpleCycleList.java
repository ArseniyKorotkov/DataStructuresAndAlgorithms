...
import track5LinkList.pack7Projects.p3.Link;

public class SimpleCycleList {

    private track5LinkList.pack7Projects.p3.Link current;
    private int size;

    public void add(long element) {
        System.out.println("add " + element);
        track5LinkList.pack7Projects.p3.Link link = new Link(element);
        if (isEmpty()) {
    ...
    public void peek() {
        if (!isEmpty())
            System.out.println("peek is " + current.getNextLink().getElement());
        else
    ...
    public void showRondo() {
        if (!isEmpty()) {
            for (int i = 0; i < size; i++) {
                System.out.print(current.getNextLink().getElement() + " ");
                current = current.getNextLink();
            }
            System.out.println();
        } else {
